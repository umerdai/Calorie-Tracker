...
const FAQs = () => {
  ...
  return (
    <div className="p-5 overflow-x-hidden"> {/* Prevent horizontal overflow */}
      ...
      <div data-aos="fade-left" className="flex flex-col space-y-4 align-middle items-center">
        {questions.map((item, index) => (
          <div key={index} className="bg-gray-100 p-2 rounded-lg shadow-md w-1/3">
            <Buttonstyle
              onClick={() => toggleAnswer(index)}
              className="flex justify-between items-center text-left font-semibold text-blue-600 hover:text-blue-800"
            >
              <span className='bg-gray-100'>{item.question}</span>
              <svg
            ...
            {openIndex === index && (
              <p className="mt-2 bg-gray-100 text-gray-700">{item.answer}</p>
            )}
          </div>
        ))}
      </div>
